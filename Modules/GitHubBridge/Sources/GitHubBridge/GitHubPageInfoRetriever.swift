...
public class GitHubPageInfoRetriever : PageInfoRetriever {
	...
	/* Note:
	 * Instead of fetching the latest object retrieved from the previous load (which forces the page info retriever to have a reference to the Core Data context and do a CD context switch),
	 *  we could use the Link header GitHub sends in its response.
	 * (Doc actually strongly advise us to do so…) */
	public func nextPageInfo(for completionResults: BridgeBackRequestResult<GitHubBMOBridge>, from pageInfo: Any, nElementsPerPage: Int) -> Any?? {
		...
		/* Note:
		 * Using a RESTMaxIdPaginator we can also get the latest retrieved id of the elements we retrieved and return a RESTMaxIdPaginatorInfo in this function.
		 * GitHub strongly recommends against manually dealing with pagination and instead use the Link header, so this is what we do.
		 * Below is an implementation of the alternative for reference.
		 * It does require the page info retriever to be allocated with a reference to the CoreData context. */
